...
6. Data yang disimpan hanya jenis event, UUID session anonim, `id_unit` opsional, flag internal, dan waktu. IP, user agent, referrer, lokasi, email, nomor WA, dan identitas customer tidak disimpan.
7. Halaman Laporan Admin/Owner menampilkan periode tetap 7 dan 30 hari: visitor unik, detail view, klik WhatsApp, klik Bagikan, rasio `whatsapp_click / detail_view`, dan lima unit dengan detail view terbanyak. Konversi WhatsApp tetap dihitung sebagai metrik terpisah (`whatsapp_click / detail_view`); klik Bagikan adalah metrik engagement pelengkap yang tidak digabung ke rasio WhatsApp. Teknisi dan publik tidak dapat membaca laporan atau tabel event mentah.
