...
- [x] Authenticated browser/visual QA hotfix UI — production `/sales/new` diuji dengan Reservasi + unit terpilih pada 1280×800, 390×844, dan 360×800. Preview unit `display:none`/`0×0` di desktop, tetap visible/readable di mobile tanpa overflow horizontal atau tertutup bottom nav; dua reviewer multimodal independen PASS tanpa blocker.
- [x] Hotfix audit log reservation `202607280001_reservation_audit_log_fix.sql` — forward-only `create_reservation`/`complete_reservation`/`refund_reservation`/`forfeit_reservation` memakai helper canonical `log_admin_action()` (`user_id`, `user_role`, `target`, `detail`), bukan kolom stale `aktor`/`aktor_role`/`target_type`/`target_id`. PGlite RED mereproduksi PostgreSQL `42703`, lalu full `npm run test:reservation`, typecheck, build, dan rollback-only smoke production lulus.

👤 **Review interface**: responsive `/sales/new` sudah lulus authenticated Playwright pada 360px/390px/desktop. Lifecycle create/complete/refund/forfeit, overdue, dan role gate lulus di PGlite; rollback-only production smoke membuktikan create + Finance + audit tanpa meninggalkan data. Owner tetap dapat melakukan review operasional dengan transaksi nyata bila diperlukan.

**Catatan hotfix 28 Juli 2026**: UI desktop dipangkas dengan menyembunyikan preview unit duplikat mulai breakpoint `md`, tanpa mengubah mobile. Migration `202607280001` memperbaiki empat audit write reservation ke schema F-SET-03 canonical. Production rollback smoke mencapai reservation + finance + audit lalu rollback penuh; count sebelum/sesudah identik (`reservations=0`, `finance=50`, `audit=11`).
...
- 2026-07-28 — **Audit reservation wajib lewat `log_admin_action()`, bukan insert kolom langsung**: schema F-SET-03 production memakai `user_id`, `user_role`, `aksi`, `target`, `detail`. Empat RPC reservation sebelumnya menulis nama kolom fiktif dari test harness (`aktor`, `aktor_role`, `target_type`, `target_id`) sehingga success path rollback dengan PostgreSQL `42703`. Forward migration mengganti write tersebut dengan helper canonical tanpa menambah kolom kompatibilitas atau mengubah money/status/role semantics; harness sekarang memakai schema production agar drift serupa gagal di test.
