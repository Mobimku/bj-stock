...
export function CancelSaleButton({ idInvoice }: { idInvoice: string }) {
  ...
  async function handleCancel() {
    setLoading(true);
    setError("");
    const res = await fetch(`/api/sales/${idInvoice}/cancel`, { method: "POST" });
    if (!res.ok) {
      const body = await res.json().catch(() => ({ error: "Gagal membatalkan invoice." }));
      setError(body.error);
      setLoading(false);
      return;
    }
      ...
      {open && (
        <dialog className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 open:flex" open onClick={(e) => { if (e.target === e.currentTarget) setOpen(false); }}>
          <div className="mx-4 w-full max-w-md rounded-2xl bg-white p-6 shadow-xl">
            <p className="text-lg font-black">Batalkan invoice?</p>
            <p className="mt-2 text-sm text-stone-600">Unit akan dikembalikan ke status <strong>Ready</strong>, garansi diakhiri, dan transaksi finance akan di-reversal. Aksi ini tidak bisa dibatalkan.</p>
