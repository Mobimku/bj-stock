-- Fase 9.14 - Tambah aktivasi Bagikan ke analytics katalog anonim.

alter table public.catalog_events
  drop constraint if exists catalog_events_event_type_check,
  drop constraint if exists catalog_events_check,
  add constraint catalog_events_event_type_check
    check (event_type in ('catalog_view', 'detail_view', 'whatsapp_click', 'share_click')),
  add constraint catalog_events_check check (
...
drop function public.get_catalog_analytics(integer);
